const ws = new WebSocket('ws://localhost:3000');

const stateEl = document.getElementById('state');
const deltaEl = document.getElementById('delta');
const timeEl = document.getElementById('time');
const logsEl = document.getElementById('logs');

const limitInput = document.getElementById('limitInput');
const applyButton = document.getElementById('applyButton');
const resetButton = document.getElementById('resetButton');

function eventToJapanese(event) {
  switch (event) {
    case "SYSTEM_START": return "起動";
    case "MOTION_SPIKE": return "衝撃検知";
    case "ENTER_PROTECT_MODE": return "保護モード移行";
    case "PROTECT_MODE_ACTIVE": return "保護中の衝撃";
    case "RULE_UPDATED": return "ルール更新";
    case "RESET": return "リセット";
    default: return event;
  }
}

function modeToJapanese(mode) {
  switch (mode) {
    case "MONITORING": return "監視中";
    case "PROTECT_MODE": return "保護モード";
    default: return mode || "-";
  }
}

applyButton.addEventListener('click', () => {
  const value = Number(limitInput.value);

  ws.send(JSON.stringify({
    type: 'SET_LIMIT',
    value
  }));
});

resetButton.addEventListener('click', () => {
  ws.send(JSON.stringify({
    type: 'RESET'
  }));
});

ws.onmessage = (event) => {
  const data = JSON.parse(event.data);

  const mode = modeToJapanese(data.mode);
  const eventName = eventToJapanese(data.event);
  const spikeCount = data.spikeCount ?? 0;
  const limit = data.protectSpikeLimit ?? Number(limitInput.value);
  const delta = typeof data.delta === 'number' ? data.delta : 0;

  limitInput.value = limit;

  stateEl.textContent = `状態: ${mode} / ${eventName} / 回数: ${spikeCount} / 条件: ${limit}回`;
  stateEl.className = data.mode === "PROTECT_MODE" ? "PROTECT_MODE" : "MONITORING";

  deltaEl.textContent = `変化量: ${delta.toFixed(3)}`;
  timeEl.textContent = `時刻: ${new Date().toLocaleTimeString()}`;

  const li = document.createElement('li');
  li.textContent = `${eventToJapanese(data.event)} | 回数 ${spikeCount}/${limit} | delta ${delta.toFixed(3)}`;
  logsEl.prepend(li);

  if (logsEl.children.length > 10) {
    logsEl.removeChild(logsEl.lastChild);
  }
};